...
class MarketRemoteDataSource implements ICachedRemoteDataSource<Market> {
  final FirebaseFirestore _firestore;
  final ICachedRemoteDataSource<List<Team>> _teamRemoteDataSource;
  Market _cachedMarket;
  ...
  @override
  Future<Either<CachedRemoteDataSourceFailure, Market>> get() async {
    ...
    return result.fold((f) => Left(f), (teams) async {
      ...
      try {
        ...
        if (snapshot.docs.isNotEmpty) {
          _cachedMarket =
              MarketDTO.fromFirestore(snapshot.docs.first).toDomain();
        } else {
          return const Left(CachedRemoteDataSourceFailure.elementNotFound());
        }
      } catch (e) {
